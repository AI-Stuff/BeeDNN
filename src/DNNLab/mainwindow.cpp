...
void MainWindow::drawLoss(const vector<float>& vfTrainLoss,const vector<float>& vfTestLoss)
{
    ...
    vector<float> x;
    for(unsigned int i=1;i<=vfTrainLoss.size();i++)
        x.push_back(i);
	...
	if (!vfTestLoss.empty())
	{
        ...
        for(unsigned int i2=1;i2<=vfTestLoss.size();i2++)
            x2.push_back(i2);
...
void MainWindow::drawAccuracy(const vector<float>& vfTrainAccuracy,const vector<float>& vfTestAccuracy)
{
    ...
    if(_pEngine->net().is_classification_mode())
    {
        ...
        //draw accuracy
        vector<float> x;
        for(unsigned int i=1;i<=vfTrainAccuracy.size();i++)
            x.push_back(i);
        ...
        {
            vector<float> x2;
            for(unsigned int i2=1;i2<=vfTestAccuracy.size();i2++)
                x2.push_back(i2);
